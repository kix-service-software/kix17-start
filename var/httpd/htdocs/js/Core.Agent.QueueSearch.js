// --
// Core.Agent.QueueSearch.js - provides the special module functions for the queue search
...
/**
 * @namespace
 * @exports TargetNS as Core.Agent.QueueSearch
 * @description
 *      This namespace contains the special module functions for the queue search.
 */
Core.Agent.QueueSearch = (function (TargetNS) {
        ...
        if (isJQueryObject($Element)) {
            $Element.autocomplete({
                ...
                source: function (Request, Response) {
                    ...
                    Core.AJAX.FunctionCall(URL, Data, function (Result) {
                        ...
                        $.each(Result, function () {
                            Data.push({
                                label: this.QueueValue + " (" + this.QueueKey + ")",
                                value: this.QueueValue
                            });
                        });
                        Response(Data);
                    });
                ...
                select: function (Event, UI) {
                    var QueueKey = UI.item.label.replace(/.*\((.*)\)$/, '$1');

                    $Element.val(UI.item.value);

                    // set hidden field SelectedQueue
                    $('#' + FieldID + 'Selected').val(QueueKey);
