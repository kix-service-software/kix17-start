...
my $StartTime;
my $TestCaseDir  = $ConfigObject->Get('Home') . '/scripts/test/GenericInterface/Operation/Ticket/TicketUpdate';
...
$StartTime           = $Self->GetMilliTimeStamp();
my $OperationModule  = 'Kernel::GenericInterface::Operation::Ticket::TicketUpdate';
...
my $TicketID = $TicketObject->TicketCreate(
    ...
    State        => 'new',
    Lock         => 'unlock',
    CustomerUser => 'max.mustermann@kixdesk.com',
    OwnerID      => 1,
    UserID       => 1,
);
if ( !$TicketID ) {
    $Self->True(
        TestName  => 'Can not create ticket',
        TestValue => $TicketID,
        StartTime => $StartTime,
    );
    # rollback transaction on database
...
for my $File ( @FilesInDirectory ) {
    ...
    $Self->TestCaseStart(
        TestCase    => 'File: ' . $File,
        Feature     => 'Kernel::GenericInterface::Operation::Ticket::TicketUpdate',
        Story       => 'GenericInterface Operation Ticket::TicketUpdate',
        Description => 'Check return data of GenericInterface Operation Ticket::TicketUpdate'
    );
