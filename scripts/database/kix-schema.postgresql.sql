...
CREATE TABLE user_preferences (
    ...
    preferences_value VARCHAR (1000) NULL
);
...
CREATE INDEX ticket_type_vid_id_name ON ticket_type (valid_id, id, name);
-- ----------------------------------------------------------
...
CREATE INDEX ticket_custid_aflag_id ON ticket (customer_id, archive_flag, id);
...
CREATE INDEX ticket_id_aflag ON ticket (id, archive_flag);
CREATE INDEX ticket_qid_stid_aflag_id ON ticket (queue_id, ticket_state_id, archive_flag, id);
...
CREATE INDEX ticket_stid_qid_id ON ticket (ticket_state_id, queue_id, id);
...
CREATE INDEX ticket_uid_stid_id ON ticket (user_id, ticket_state_id, id);
...
CREATE INDEX ticket_flag_ticket_key ON ticket_flag (ticket_key);
CREATE INDEX ticket_flag_tkey_tval_crby_tid ON ticket_flag (ticket_key, ticket_value, create_by, ticket_id);
-- ----------------------------------------------------------
...
CREATE INDEX ticket_hist_htid_tid_stid_ct ON ticket_history (history_type_id, ticket_id, state_id, create_time);
...
CREATE INDEX ticket_history_tid_stid_htid ON ticket_history (ticket_id, state_id, history_type_id);
...
CREATE INDEX article_flag_artkey_crby_aid ON article_flag (article_key, create_by, article_id);
...
CREATE INDEX article_search_incoming_time ON article_search (incoming_time);
...
CREATE TABLE sla (
    ...
    change_by INTEGER NOT NULL,
    type_id INTEGER NULL,
    min_time_bet_incidents INTEGER NULL,
    PRIMARY KEY(id),
...
CREATE INDEX link_relation_src_key ON link_relation (source_key);
CREATE INDEX link_relation_src_key_trgt_key ON link_relation (source_key, target_key);
CREATE INDEX link_relation_src_obj_id ON link_relation (source_object_id);
CREATE INDEX link_relation_target_key ON link_relation (target_key);
CREATE INDEX link_relation_target_obj_id ON link_relation (target_object_id);
-- ----------------------------------------------------------
...
CREATE INDEX xml_storage_type_ckey_cval ON xml_storage (xml_type, xml_content_key, xml_content_value);
...
CREATE TABLE faq_state_type (
    ...
    name VARCHAR (200) NOT NULL,
    PRIMARY KEY(id),
    CONSTRAINT faq_state_type_name UNIQUE (name)
);
-- ----------------------------------------------------------
...
CREATE TABLE faq_state (
    ...
    PRIMARY KEY(id),
    CONSTRAINT faq_state_name UNIQUE (name)
);
...
CREATE TABLE faq_item (
    ...
    content_type VARCHAR (250) DEFAULT 'text/html' NOT NULL,
    f_keywords VARCHAR (400) NULL,
    f_field1 VARCHAR NULL,
    f_field2 VARCHAR NULL,
    f_field3 VARCHAR NULL,
    f_field4 VARCHAR NULL,
    f_field5 VARCHAR NULL,
    f_field6 VARCHAR NULL,
    created timestamp(0) NOT NULL,
    created_by INTEGER NOT NULL,
    changed timestamp(0) NOT NULL,
    changed_by INTEGER NOT NULL,
...
CREATE TABLE faq_attachment (
    ...
    content_type VARCHAR (250) NULL,
    content TEXT NOT NULL,
    inlineattachment SMALLINT DEFAULT 0 NOT NULL,
    created timestamp(0) NOT NULL,
    created_by INTEGER NOT NULL,
    changed timestamp(0) NOT NULL,
    changed_by INTEGER NOT NULL,
...
CREATE TABLE general_catalog (
    ...
    change_by INTEGER NOT NULL,
    PRIMARY KEY(id),
    CONSTRAINT general_catalog_U_803 UNIQUE (general_catalog_class, name)
);
-- ----------------------------------------------------------
...
CREATE TABLE configitem (
    ...
    cur_depl_state_id INTEGER NULL,
    cur_inci_state_id INTEGER NULL,
    create_time timestamp(0) NOT NULL,
    create_by INTEGER NOT NULL,
    change_time timestamp(0) NOT NULL,
    change_by INTEGER NOT NULL,
    PRIMARY KEY(id),
    CONSTRAINT configitem_U_210 UNIQUE (configitem_number)
);
...
CREATE TABLE configitem_history_type (
    ...
    change_by INTEGER NULL,
    PRIMARY KEY(id),
    CONSTRAINT configitem_history_type_name UNIQUE (name)
);
-- ----------------------------------------------------------
...
CREATE INDEX index_attachment_directory_backend ON attachment_directory (storage_backend);
-- ----------------------------------------------------------
...
CREATE TABLE attachment_storage (
    ...
    data TEXT NULL,
    PRIMARY KEY(id),
    CONSTRAINT attachment_storage_U_166 UNIQUE (attachment_directory_id)
);
CREATE INDEX attachment_storage_dir_id ON attachment_storage (attachment_directory_id);
-- ----------------------------------------------------------
--  create table standard_templatex
-- ----------------------------------------------------------
CREATE TABLE standard_templatex (
    template_id INTEGER NOT NULL,
    t_to VARCHAR (3800) NULL,
    t_cc VARCHAR (3800) NULL,
    t_bcc VARCHAR (3800) NULL,
    t_ticket_state_id SMALLINT NULL,
    t_pending_time INTEGER NULL,
    t_pending_type INTEGER NULL,
    PRIMARY KEY(template_id)
);
-- ----------------------------------------------------------
--  create table kix_article_flag
-- ----------------------------------------------------------
CREATE TABLE kix_article_flag (
    article_id BIGINT NOT NULL,
    article_key VARCHAR (50) NOT NULL,
...
-- ----------------------------------------------------------
--  create table kix_customer_company_prefs
-- ----------------------------------------------------------
CREATE TABLE kix_customer_company_prefs (
    user_id VARCHAR (250) NOT NULL,
    preferences_key VARCHAR (100) NOT NULL,
    preferences_value VARCHAR NULL
);
-- ----------------------------------------------------------
--  create table kix_dep_dynamic_field
-- ----------------------------------------------------------
CREATE TABLE kix_dep_dynamic_field (
    id INTEGER NOT NULL,
    name VARCHAR (150) NOT NULL,
...
-- ----------------------------------------------------------
--  create table kix_dep_dynamic_field_prefs
-- ----------------------------------------------------------
CREATE TABLE kix_dep_dynamic_field_prefs (
    id serial NOT NULL,
    dependingfield_id INTEGER NOT NULL,
...
CREATE TABLE kix_file_watcher (
    ...
    path_lower VARCHAR (255) NULL,
    name VARCHAR (255) NULL,
    name_lower VARCHAR (255) NULL,
    first_found INTEGER NULL,
    last_found INTEGER NULL,
    last_mod INTEGER NULL,
    mod_type INTEGER NULL,
    filesize BIGINT NULL,
    outdated INTEGER NULL,
    PRIMARY KEY(id)
);
CREATE INDEX kix_file_watcher_filesize ON kix_file_watcher (filesize);
CREATE INDEX kix_file_watcher_fingerprint ON kix_file_watcher (fingerprint);
CREATE INDEX kix_file_watcher_last_mod ON kix_file_watcher (last_mod);
CREATE INDEX kix_file_watcher_name ON kix_file_watcher (name);
CREATE INDEX kix_file_watcher_name_lower ON kix_file_watcher (name_lower);
CREATE INDEX kix_file_watcher_parent_id ON kix_file_watcher (parent_id);
CREATE INDEX kix_file_watcher_path ON kix_file_watcher (path);
-- ----------------------------------------------------------
--  create table kix_text_module
-- ----------------------------------------------------------
CREATE TABLE kix_text_module (
    id serial NOT NULL,
    ...
    f_agent INTEGER DEFAULT 0 NULL,
    f_customer INTEGER DEFAULT 0 NULL,
    f_public INTEGER DEFAULT 0 NULL,
    valid_id INTEGER NOT NULL,
    create_time timestamp(0) NOT NULL,
    create_by INTEGER NOT NULL,
    change_time timestamp(0) NOT NULL,
...
CREATE INDEX kix_text_module_comment1 ON kix_text_module (comment1);
CREATE INDEX kix_text_module_comment2 ON kix_text_module (comment2);
CREATE INDEX kix_text_module_keywords ON kix_text_module (keywords);
CREATE INDEX kix_text_module_language ON kix_text_module (language);
CREATE INDEX kix_text_module_name ON kix_text_module (name);
-- ----------------------------------------------------------
--  create table kix_text_module_object_link
-- ----------------------------------------------------------
CREATE TABLE kix_text_module_object_link (
    text_module_id INTEGER NOT NULL,
...
CREATE INDEX kix_text_module_object_link_object_id ON kix_text_module_object_link (object_id);
CREATE INDEX kix_text_module_object_link_object_type ON kix_text_module_object_link (object_type);
-- ----------------------------------------------------------
--  create table kix_ticket_notes
-- ----------------------------------------------------------
CREATE TABLE kix_ticket_notes (
    ticket_id BIGINT NOT NULL,
...
-- ----------------------------------------------------------
--  create table kix_ticket_template
-- ----------------------------------------------------------
CREATE TABLE kix_ticket_template (
    id serial NOT NULL,
    name VARCHAR (150) NOT NULL,
    ...
    change_time timestamp(0) NOT NULL,
    change_by INTEGER NOT NULL,
    f_agent INTEGER NULL,
    f_customer INTEGER NULL,
    PRIMARY KEY(id),
    CONSTRAINT kix_ticket_template_name UNIQUE (name)
);
-- ----------------------------------------------------------
--  create table kix_ticket_template_prefs
-- ----------------------------------------------------------
CREATE TABLE kix_ticket_template_prefs (
    template_id INTEGER NOT NULL,
    preferences_key VARCHAR (100) NOT NULL,
...
-- ----------------------------------------------------------
--  create table kix_text_module_category
-- ----------------------------------------------------------
CREATE TABLE kix_text_module_category (
    id serial NOT NULL,
...
-- ----------------------------------------------------------
--  create table kix_search_profile
-- ----------------------------------------------------------
CREATE TABLE kix_search_profile (
...
CREATE INDEX kix_search_profile_nme_st_log ON kix_search_profile (name, state, login);
-- ----------------------------------------------------------
--  create table kix_link_graph
-- ----------------------------------------------------------
CREATE TABLE kix_link_graph (
    id serial NOT NULL,
    name VARCHAR (250) NOT NULL,
...
CREATE INDEX kix_link_graph_object_id_object_type ON kix_link_graph (object_id, object_type);
-- ----------------------------------------------------------
--  create table kix_ticket_checklist
-- ----------------------------------------------------------
CREATE TABLE kix_ticket_checklist (
    id serial NOT NULL,
    ticket_id BIGINT NOT NULL,
...
CREATE INDEX kix_ticket_checklist_ticket_id ON kix_ticket_checklist (ticket_id);
-- ----------------------------------------------------------
--  create table overlay_agent
-- ----------------------------------------------------------
CREATE TABLE overlay_agent (
    id serial NOT NULL,
    subject VARCHAR (250) NOT NULL,
