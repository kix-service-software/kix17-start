...
sub CustomerSearch {
    ...
    if ( $Param{SearchFields} ) {
        ...
        # backend mapping
        if ( $Self->{CustomerUserMap}->{CustomerUserFurtherSearchFields} ) {
            $SearchFurtherFields = $Self->{CustomerUserMap}->{CustomerUserFurtherSearchFields};
        }

        # fallback, if no individual mapping
        else {
            $SearchFurtherFields = $Kernel::OM->Get('Kernel::Config')->Get('FurtherSearchFields::Mapping');
        }
...
sub CustomerUserDataGet {
    ...
    my $CustomerUserDNQuote                 = $CustomerUserDN;
    $CustomerUserDNQuote =~ s/\\/\\\\/g;
    $CustomerUserDNQuote =~ s/\(/\\(/g;
    $CustomerUserDNQuote =~ s/\)/\\)/g;
