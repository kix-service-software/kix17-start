# --
# based upon Kernel/System/DynamicField/Driver/ObjectReference/Multiselect.pm
# original Copyright (C) 2001-2013 OTRS AG, http://otrs.org/
...
package Kernel::System::DynamicField::Driver::ObjectReference::CustomerUser;
...
sub new {
    ...
    my $DynamicFieldDriverExtensions
        = $Kernel::OM->Get('Kernel::Config')->Get('DynamicFields::Extension::Driver::CustomerUser');
...
sub EditFieldRender {
    ...
    if ( $DisplayFieldType eq 'AutoComplete' ) {
        ...
        $FieldClass .= ' CustomerUserAutoComplete ';
...
EOF
        ...
        my $FieldSelector = '.CustomerUserAutoComplete';
...
EOF
        ...
        # get data
        my %ObjectList = $Self->{CustomerUserObject}->CustomerUserList(
            Valid => 1,
        );
...
sub PossibleValuesGet {
        ...
        || $Param{GetAutocompleteValues} )
    {
        ...
        my %ObjectList;
        if ($Param{Search}) {
            %ObjectList = $Self->{CustomerUserObject}->CustomerSearch(
                Search => $Param{Search},
            );
        }
        else {
            %ObjectList = $Self->{CustomerUserObject}->CustomerUserList(
                Valid => 1,
            );
        }
