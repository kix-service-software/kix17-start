...
=item _MigrateTemplateData()

Migrates old SysConfig TicketTemplate data to database

    my $MigrationResult = $TicketTemplateObject->_MigrateTemplateData(
    );

=cut

sub _MigrateTemplateData {
    my ( $Self, %Param ) = @_;
    ...
    # get list of existing ticket template names (agent)
    my %AgentTemplateData;
    for my $DataKey (
        qw(AgentTicketPhone AgentTicketEmail AgentTicketPhoneQuick AgentTicketEmailQuick)
        )
    {

        next
            if !(
            $Self->{ConfigObject}->Get('Ticket::QuickTicketByDefaultSet::Selection')
            ->{$DataKey}
            );
        my $AgentQuickTicketConfig =
            $Self->{ConfigObject}->Get('Ticket::QuickTicketByDefaultSet::Selection')->{$DataKey};
        for my $TemplateUniqueName ( keys %{$AgentQuickTicketConfig} ) {
            next if defined $AgentTemplateData{$TemplateUniqueName}->{Name};
            my %Temp;
            $Temp{Name}  = $AgentQuickTicketConfig->{$TemplateUniqueName};
            $Temp{Agent} = 1;
            $AgentTemplateData{$TemplateUniqueName} = \%Temp;
        }
    }

    # get ticket template data
    my $AgentQuickTicketData = $Self->{ConfigObject}->Get('Ticket::QuickTicketByDefaultSet');

    if ( !$AgentQuickTicketData || ref($AgentQuickTicketData) ne 'HASH' ) {
        $Kernel::OM->Get('Kernel::System::Log')->Log(
            Priority => 'error',
            Message  => "Looking for default set, but not defined!",
        );
    }
    else {
        for my $FieldKey ( keys %{$AgentQuickTicketData} ) {

            for my $QuickTicketTemplateName ( keys %{ $AgentQuickTicketData->{$FieldKey} } ) {
                my $TicketTemplateName;
                my $TicketTemplateSubName = '';
                my $Separator             = '';
                if ( $QuickTicketTemplateName =~ /(.*)::(.*)/ ) {
                    $TicketTemplateName    = $1;
                    $TicketTemplateSubName = $2;
                    if (
                        $FieldKey ne 'TicketFreeText'
                        && $FieldKey ne 'TicketFreeTime'
                        )
                    {
                        $Separator = '::';
                    }
                    ...
                    $TicketTemplateName = $QuickTicketTemplateName;
                }
                next if !defined $AgentTemplateData{$TicketTemplateName};
                $AgentTemplateData{$TicketTemplateName}
                    ->{ $FieldKey . $Separator . $TicketTemplateSubName } =
                    $AgentQuickTicketData->{$FieldKey}->{$QuickTicketTemplateName};
            }
        }
    }

    #save
    for my $TicketTemplate ( keys %AgentTemplateData ) {

        # add TypeID, QueueID, PriorityID,.... remove old values
        if ( $AgentTemplateData{$TicketTemplate}->{TicketType} ) {
            $AgentTemplateData{$TicketTemplate}->{TypeID}
                = $Self->{TypeObject}
                ->TypeLookup( Type => $AgentTemplateData{$TicketTemplate}->{TicketType} );
            delete $AgentTemplateData{$TicketTemplate}->{TicketType};
        }
        if ( $AgentTemplateData{$TicketTemplate}->{Queue} ) {
            $AgentTemplateData{$TicketTemplate}->{QueueID}
                = $Self->{QueueObject}
                ->QueueLookup( Queue => $AgentTemplateData{$TicketTemplate}->{Queue} );
            delete $AgentTemplateData{$TicketTemplate}->{Queue};
        }
        if ( $AgentTemplateData{$TicketTemplate}->{Priority} ) {
            $AgentTemplateData{$TicketTemplate}->{ProrityID}
                = $Self->{PriorityObject}
                ->PriorityLookup( Priority => $AgentTemplateData{$TicketTemplate}->{Priority} );
            delete $AgentTemplateData{$TicketTemplate}->{Priority};
        }
        if ( $AgentTemplateData{$TicketTemplate}->{SLA} ) {
            $AgentTemplateData{$TicketTemplate}->{SLAID}
                = $Self->{SLAObject}
                ->SLALookup( Name => $AgentTemplateData{$TicketTemplate}->{SLA} );
            delete $AgentTemplateData{$TicketTemplate}->{SLA};
        }
        if ( $AgentTemplateData{$TicketTemplate}->{Service} ) {
            $AgentTemplateData{$TicketTemplate}->{ServiceID}
                = $Self->{ServiceObject}
                ->ServiceLookup( Name => $AgentTemplateData{$TicketTemplate}->{Service} );
            delete $AgentTemplateData{$TicketTemplate}->{Service};
        }
        if ( $AgentTemplateData{$TicketTemplate}->{State} ) {
            $AgentTemplateData{$TicketTemplate}->{StateID}
                = $Self->{StateObject}
                ->StateLookup( State => $AgentTemplateData{$TicketTemplate}->{State} );
            delete $AgentTemplateData{$TicketTemplate}->{State};
        }
        if (
            defined $AgentTemplateData{$TicketTemplate}->{LinkDirection}
            && $AgentTemplateData{$TicketTemplate}->{LinkDirection} eq 'Source'
            )
        {
            $AgentTemplateData{$TicketTemplate}->{LinkDirection} = 'Parent';
        }
        elsif (
            defined $AgentTemplateData{$TicketTemplate}->{LinkDirection}
            && $AgentTemplateData{$TicketTemplate}->{LinkDirection} eq 'Target'
            )
        {
            $AgentTemplateData{$TicketTemplate}->{LinkDirection} = 'Child';
        }

        $AgentTemplateData{$TicketTemplate}->{ID} = $Self->TicketTemplateCreate(
            Data   => $AgentTemplateData{$TicketTemplate},
            UserID => 1,
            Name   => $AgentTemplateData{$TicketTemplate}->{Name},
        );
    }
    ...
    my %CustomerTemplateData;

    # get list of existing ticket template names (customer)
    my $CustomerQuickTicketData =
        $Self->{ConfigObject}->Get('Ticket::QuickTicketByDefaultSet::Customer');

    for my $TemplateUniqueName ( keys %{ $CustomerQuickTicketData->{TemplateRegistration} } ) {
        my %Temp;
        $Temp{Name}     = $TemplateUniqueName;
        $Temp{Customer} = 1;
        $CustomerTemplateData{
            $CustomerQuickTicketData->{TemplateRegistration}
                ->{$TemplateUniqueName}
            }
            = \%Temp;
    }

    for my $FieldKey ( keys %{$CustomerQuickTicketData} ) {
        for my $QuickTicketTemplateName ( keys %{ $CustomerQuickTicketData->{$FieldKey} } ) {
            my $TicketTemplateName;
            my $TicketTemplateSubName = '';
            my $Separator             = '';
            if ( $QuickTicketTemplateName =~ /(.*)::(.*)/ ) {
            ...
            next if !defined $CustomerTemplateData{$TicketTemplateName};
            $CustomerTemplateData{$TicketTemplateName}
                ->{ $FieldKey . $Separator . $TicketTemplateSubName } =
                $CustomerQuickTicketData->{$FieldKey}->{$QuickTicketTemplateName};
        }

    }

    #save
    for my $TicketTemplate ( keys %CustomerTemplateData ) {

        # add TypeID, QueueID, PriorityID,.... remove old values
        if ( $CustomerTemplateData{$TicketTemplate}->{TicketType} ) {
            $CustomerTemplateData{$TicketTemplate}->{TypeID}
                = $Self->{TypeObject}
                ->TypeLookup( Type => $CustomerTemplateData{$TicketTemplate}->{TicketType} );
            delete $CustomerTemplateData{$TicketTemplate}->{TicketType};
        }
        if ( $CustomerTemplateData{$TicketTemplate}->{Queue} ) {
            $CustomerTemplateData{$TicketTemplate}->{QueueID}
                = $Self->{QueueObject}
                ->QueueLookup( Queue => $CustomerTemplateData{$TicketTemplate}->{Queue} );
            delete $CustomerTemplateData{$TicketTemplate}->{Queue};
        }
        if ( $CustomerTemplateData{$TicketTemplate}->{Priority} ) {
            $CustomerTemplateData{$TicketTemplate}->{ProrityID}
                = $Self->{PriorityObject}
                ->PriorityLookup( Priority => $CustomerTemplateData{$TicketTemplate}->{Priority} );
            delete $CustomerTemplateData{$TicketTemplate}->{Priority};
        }
        if ( $CustomerTemplateData{$TicketTemplate}->{SLA} ) {
            $CustomerTemplateData{$TicketTemplate}->{SLAID}
                = $Self->{SLAObject}
                ->SLALookup( Name => $CustomerTemplateData{$TicketTemplate}->{SLA} );
            delete $CustomerTemplateData{$TicketTemplate}->{SLA};
        }
        if ( $CustomerTemplateData{$TicketTemplate}->{Service} ) {
            $CustomerTemplateData{$TicketTemplate}->{ServiceID}
                = $Self->{ServiceObject}
                ->ServiceLookup( Name => $CustomerTemplateData{$TicketTemplate}->{Service} );
            delete $CustomerTemplateData{$TicketTemplate}->{Service};
        }
        if ( $CustomerTemplateData{$TicketTemplate}->{State} ) {
            $CustomerTemplateData{$TicketTemplate}->{StateID}
                = $Self->{StateObject}
                ->StateLookup( State => $CustomerTemplateData{$TicketTemplate}->{State} );
            delete $CustomerTemplateData{$TicketTemplate}->{State};
        }

        $CustomerTemplateData{$TicketTemplate}->{ID} = $Self->TicketTemplateCreate(
            Data   => $CustomerTemplateData{$TicketTemplate},
            UserID => 1,
            Name   => $CustomerTemplateData{$TicketTemplate}->{Name},
        );
    }
