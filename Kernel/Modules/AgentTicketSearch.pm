...
sub Run {
    ...
    # get search string params (get submitted params)
        ...
        for my $Key (
            ...
            PendingTimeSearchType EscalationTimeSearchType
    ...
    if ( $Self->{Subaction} eq 'Search' && !$Self->{EraseTemplate} ) {
        ...
        if (
            ...
            my %SearchProfileCategory = $SearchProfileObject->SearchProfileCategoryGet(
                Name      => $SearchProfileName,
                UserLogin => $Self->{UserLogin},
            );
            if ( defined $SearchProfileCategory{Category} && $SearchProfileCategory{Category} ) {
                $SearchProfileObject->SearchProfileCategoryDelete(
                    Category  => $SearchProfileCategory{Category},
                    Name      => $SearchProfileName,
                );
            }
            ...
            if ( $Self->{Profile} ne 'last-search' && $GetParam{SearchProfileShare} ) {
                ...
                if ( $GetParam{SearchProfileCategory} ) {
                    $Category = $GetParam{SearchProfileCategory};
                }

                # new category
                elsif ( $GetParam{SearchProfileAddCategory} ) {
                    $Category = $GetParam{SearchProfileAddCategory};
                }
        ...
        if ( $GetParam{LinkedTickets} ) {
            ...
            # get linked tickets
            my @LinkedTicketIDs = $TicketObject->GetLinkedTickets(
                UserID   => $Self->{UserID},
                Customer => $GetParam{CustomerUser},
            );

            # check which tickets are linked
            my @ViewableTicketsTmp;
            for my $TicketID (@ViewableTicketIDs) {
                next if !grep { $_ eq $TicketID } @LinkedTicketIDs;
        ...
        if ($SearchProfileUser) {
            ...
            # remove shared profile and all copied from other agents
            elsif (
                defined $SearchProfileCategory{State}
        ...
        push @Attributes, (
            ...
            },
            {
                ...
                Value => 'Ticket Pending Time (between)',
            },
            {
                Key   => 'ArticleFlag',
                Value => 'Article Flag',
        ...
        my %ArticleFlagList = ();
        if ( defined $ArticleConfig->{ArticleFlags} && ref $ArticleConfig->{ArticleFlags} eq 'HASH' ) {
            %ArticleFlagList = %{ $ArticleConfig->{ArticleFlags} };
        }
