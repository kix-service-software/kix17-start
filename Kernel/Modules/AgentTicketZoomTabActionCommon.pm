# --
# based upon AgentTicketActionCommon.pm
...
package Kernel::Modules::AgentTicketZoomTabActionCommon;
...
sub Run {
    ...
    if ( $Self->{PretendAction} ) {
        if ( !$ConfigObject->Get("Ticket::Frontend::$Self->{PretendAction}") ) {
            $Kernel::OM->Get('Kernel::System::Log')->Log(
                Priority => 'error',
                Message  => "Kernel::Module::AgentTicketZoomTabActionCommon: PretendAction "
    ...
    if ( !$GetParam{NewQueueID} && $GetParam{DestQueue} ) {
        $GetParam{NewQueueID} = QueueObject->QueueLookup( Queue => $GetParam{DestQueue} );
    }
...
# ---
    ...
    # KIX4OTRS-capeIT
        ...
        for my $Attachment (@Attachments)
        {
            $Content
                ...
                . $Attachment->{Filesize} . ')';
            $Content
                .= "&nbsp;<button class='AttachmentDeleteButton' type='button' id='AttachmentDelete"
                . $Attachment->{FileID}
    ...
    elsif ( $Self->{Subaction} eq 'AJAXRemoveUpload' ) {
        my $FileID = $ParamObject->GetParam( Param => 'AttachmentDeleteID' );
        $UploadCacheObject->FormIDRemoveFile(
            FormID => $Self->{FormID},
            FileID => $FileID,
        );
