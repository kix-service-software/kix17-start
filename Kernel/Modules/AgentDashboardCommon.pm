            ...
            for my $Param ( @{ $Element{Preferences} } ) {
                ...
                # snipplet here instead of in some DTL affects less files
                    ...
                    $Param->{Option} = '<input type="checkbox" '
                        . ' id="' . $Param->{Name} . '" '
                        . ' name="' . $Param->{Name} . '" '
                        . ' value="' . ( $Param->{SelectedID} || '' ) . '" '
                        . " $CheckedFlag />";
                    $Param->{Block} = 'Option';
                }

                elsif ( $Param->{Block} eq 'Sortable' ) {
                    $Param->{Option} =
                        ...
                        . '" id="'
                        . $Param->{Name} . '" />'
                        . '<ul class="sortable ColumnOrder" id="' . $Param->{Name} . 'All">';

                    if ( $Param->{Data} ) {
                        for my $ColRef ( @{ $Param->{Data} } ) {
                            my $CheckedFlag = "";
                            my $CSSClass    = '';
                            if ( $ColRef->{Selected} ) {
                                $CSSClass    = ' Checked';
                            ...
                            $Param->{Option} = $Param->{Option}
                                ...
                                . $ColRef->{Column} . '">'
                                . '<input type="checkbox" '
                                . ' id="' . $Param->{Name} . '-Show' . $ColRef->{Column} . '" '
                                . ' name="' . $Param->{Name} . '-Show' . $ColRef->{Column} . '" '
                                . ' value="' . $ColRef->{Selected} . '" '
                                . " $CheckedFlag />"
                                . $ColRef->{ColumnTitle}
