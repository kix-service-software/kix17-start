...
sub SendEmail {
    ...
    if (%Error) {
        ...
        %GetParamExtended = $Self->_GetExtendedParams();
        %GetParam         = %{ $GetParamExtended{GetParam} };
        ...
        if ( $GetParam{ForwardTemplateID} ) {
            ...
            # get template
            $Data{StdTemplate} = $TemplateGeneratorObject->Template(
                TicketID   => $Self->{TicketID},
                ArticleID  => $Data{ArticleID},
                TemplateID => $GetParam{ForwardTemplateID},
                Data       => \%Data,
        ...
        if ( $LayoutObject->{BrowserRichText} ) {
            ...
            my $ForwardedMessageFrom
                = $LayoutObject->{LanguageObject}->Translate('Forwarded message from');
            my $EndForwardedMessage
                = $LayoutObject->{LanguageObject}->Translate('End forwarded message');
    ...
    if ( IsArrayRefWithData( $Config->{ArticleTypes} ) ) {
        ...
        if ( $Self->{GetParam}->{ArticleTypeID} ) {
            ...
            if ( $Selected{SelectedValue} !~ m{internal} ) {
                if ( $Self->{GetParam}->{ArticleID} ) {
                    my %ArticleData = $TicketObject->ArticleGet(
                        ArticleID     => $Self->{GetParam}->{ArticleID},
                        DynamicFields => 0,
                    );
